package cn.shopee.common.mybatis.mvc.parse;

import cn.shopee.common.query.data.Queryable;
import cn.shopee.common.query.data.Condition;
import cn.shopee.common.query.data.Condition.Filter;
import cn.shopee.common.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.shopee.common.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.shopee.common.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.shopee.common.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.shopee.common.query.data.Sort.Direction.ASC) {
